...
int main() {
...

    {
        ...
        require(nano_lance::blob_v2_unpack_descriptor_row(packed, roundtrip, error), error);
...

    {
        ...
        for (std::size_t row = 0; row < column_values.blob_v2.row_packed_sizes.size(); ++row) {
            ...
            require(nano_lance::blob_v2_unpack_descriptor_row(row_bytes, unpacked, error), error);
            if (row == 0) {
        ...
        require(nano_lance::blob_v2_control_buffer_to_row_sizes(control, 2, roundtrip_sizes, error), error);
