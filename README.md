...
Built on the **[nanotins](https://github.com/yoavbendor/nanotins)** stack — a sister project, vendored here as a git submodule under [`extern/nanotins`](extern/nanotins/) — two layered, reusable libraries: **soatins** (reflection: describe a struct once → SoA + Arrow) and **nanotins** (pcap/pcapng + L2/L3/L4 protocol decode + the wire_spec declarative wire-parsing core + the spec_dag DAG/FSM dispatcher). The [`examples/pcapng2lance`](examples/pcapng2lance/) worked example glues them together into a streaming capture → Lance converter. (A GPU/CUDA executor layer is a planned future addition, developed separately.)
