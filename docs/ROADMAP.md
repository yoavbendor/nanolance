...
| F1 | *(status)* **Measured and closed, no change.** Bitpacked columns were already written as 32,768-row multi-chunk pages; only flat columns keep 4,095-row pages. Warmed-up medians (21 reads, 200,000 rows): rust lance reads nanolance's files within 0-33% of its own (≤1.3 ms), faster on floats; routing flat columns to multi-chunk pages did not move it. The 6x gap quoted earlier was a cold first read. Bigger pages would cost write-side memory, which F3 bounds instead. | | |
