...
// Minimal, dependency-light S3 object reader. nanolance only ever needs to *read* ranged slices out of
// external blobs (see src/nano_lance_external_blob.cpp), so this provides exactly that — a seekable,
// read-ahead-buffered std::istream over S3 range GETs — using nothing but libcurl + OpenSSL instead of the
// full AWS SDK for C++. It is a drop-in for the legacy `AwsSdkStreamFactory` seam: same open()/error().
//
...
#ifndef NANOLANCE_S3_MIN_READER_H
#define NANOLANCE_S3_MIN_READER_H
...
#endif  // NANOLANCE_S3_MIN_READER_H
