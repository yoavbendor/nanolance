...
struct WireInt {
    soatins::be<std::uint16_t> len;  // big-endian on the wire
    soatins::be<std::uint32_t> id;
    soatins::le<std::uint32_t> tag;  // little-endian on the wire
};
BOOST_DESCRIBE_STRUCT(WireInt, (), (len, id, tag))

struct Flags {
    soatins::bits<soatins::be<std::uint16_t>, soatins::field<"pcp", 3>, soatins::field<"dei", 1>,
                   soatins::field<"vid", 12>>
        tci;
    std::uint8_t trailer;
};
...
template <class T>
void write_dataset(const soatins::soa<T>& s, const std::filesystem::path& dir) {
    std::error_code ec;
    ...
    ArrowArray batch{};
    std::string err;
    require(soatins::arrow_schema<T>(schema, err), ("arrow_schema: " + err).c_str());
    require(soatins::to_arrow<T>(s, batch, err), ("to_arrow: " + err).c_str());
